...
public class Guitar extends StringInstrument {
	...
	public Guitar(String brand, Number price, int numOfStrings, String type) {
		...
		setType(type);
		checkNumOfStrings(numOfStrings);
	}
	...
	private void checkNumOfStrings(int numOfStrings) {
		switch (indexOfType()) {
		case CLASSIC:
		...
		case ELECTRIC:
			if (numOfStrings < ELEC_MIN_NUM_OF_STRINGS || numOfStrings > ELEC_MAX_NUM_OF_STRINGS)
				throw new InputMismatchException("Electric Guitars have minimum 6 strings, not " + numOfStrings);
			break;
		}
	}
	...
	@Override
	public Guitar clone() {
		Guitar clonedGuitar = new Guitar(getBrand(), getPrice(), getNumOfStrings(), getType());
		return clonedGuitar;
	}
